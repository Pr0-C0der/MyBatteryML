# PowerShell script to run normalized capacity vs voltage analysis
...
if ($Output -eq "") {
    if ($All) {
        $Output = "normalized_capacity_voltage_all_${DatasetName}"
    }
    elseif ($MultipleBatteries) {
        $Output = "normalized_capacity_voltage_multiple_${DatasetName}.png"
    }
    else {
        $Output = "normalized_capacity_voltage_${DatasetName}.png"
    }
}

Write-Host "Running Normalized Capacity vs Voltage Analysis..." -ForegroundColor Green
...
$pythonArgs = @(
    "batteryml/chemistry_data_analysis/statistical_analysis/normalized_capacity_voltage.py",
    $DatasetName,
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "Successfully created normalized capacity vs voltage plot!" -ForegroundColor Green
