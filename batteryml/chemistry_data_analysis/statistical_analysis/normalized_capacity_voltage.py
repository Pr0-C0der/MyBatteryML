#!/usr/bin/env python3
"""
Normalized Discharge Capacity vs Normalized Voltage Analysis

This module provides functionality to plot normalized discharge capacity vs normalized voltage
...
def normalize_capacity_voltage(data: pd.DataFrame, battery_id: str) -> pd.DataFrame:
    """
    Normalize discharge capacity and voltage for a specific battery.
    ...
    discharge_data = battery_data[battery_data['current'] < 0].copy()
    
    if discharge_data.empty:
        raise ValueError(f"No discharge data found for battery {battery_id}")
    
    # Calculate cumulative capacity (discharge capacity)
    discharge_data = discharge_data.sort_values('time')
    # Use actual time differences, not diff().fillna(0)
    time_diffs = discharge_data['time'].diff()
    time_diffs.iloc[0] = 0  # First time step is 0
    discharge_data['cumulative_capacity'] = np.cumsum(discharge_data['current'] * time_diffs / 3600)  # Convert to Ah
    
    # Normalize capacity (divide by maximum capacity)
    max_capacity = discharge_data['cumulative_capacity'].max()
    discharge_data['normalized_capacity'] = discharge_data['cumulative_capacity'] / max_capacity
    
    # Normalize voltage (divide by maximum voltage)
    max_voltage = discharge_data['voltage'].max()
    discharge_data['normalized_voltage'] = discharge_data['voltage'] / max_voltage
    
    return discharge_data


def plot_normalized_capacity_voltage(data: pd.DataFrame, 
                                   dataset_name: str,
                                   cycle_limit: int = 10,
    ...
    """
    Plot normalized discharge capacity vs normalized voltage for multiple cycles.
    ...
    for i, cycle in enumerate(cycles_to_plot):
        try:
            ...
            normalized_data = normalize_capacity_voltage(cycle_data, battery_id)
            
            if normalized_data.empty:
                print(f"Warning: No discharge data for cycle {cycle}")
                continue
    ...
    plt.ylabel('Normalized Discharge Capacity', fontsize=12)
    plt.title(f'Normalized Voltage vs Normalized Discharge Capacity\nBattery: {battery_id} | Dataset: {dataset_name}', 
...
def plot_multiple_batteries(data: pd.DataFrame, 
    ...
    """
    Plot normalized capacity vs voltage for multiple batteries.
    ...
    for battery_idx, battery_id in enumerate(available_batteries):
        ...
        ax.set_ylabel('Normalized Discharge Capacity', fontsize=10)
    ...
    # Overall title
    fig.suptitle(f'Normalized Voltage vs Normalized Discharge Capacity\nDataset: {dataset_name}', 
...
def plot_all_batteries(data: pd.DataFrame, 
                      ...
                      figsize: Tuple[int, int] = (12, 8)) -> None:
    """
    Plot normalized voltage vs capacity for all batteries in the dataset.
    ...
    if output_dir is None:
        output_dir = f"normalized_capacity_voltage_all_{dataset_name}"
    ...
    for battery_id in available_batteries:
        try:
            ...
            battery_output_path = output_path / f"normalized_capacity_voltage_{battery_id}.png"
            plt.tight_layout()
...
def main():
    """Main function to run the normalized capacity vs voltage analysis."""
    parser = argparse.ArgumentParser(description='Plot normalized discharge capacity vs normalized voltage')
    ...
    try:
        ...
        if output_path is None:
            if args.all:
                output_path = f"normalized_capacity_voltage_all_{args.dataset_name}.png"
            elif args.multiple_batteries:
                output_path = f"normalized_capacity_voltage_multiple_{args.dataset_name}.png"
            else:
                output_path = f"normalized_capacity_voltage_{args.dataset_name}.png"
        ...
        if args.all:
            ...
            plot_normalized_capacity_voltage(data, args.dataset_name, args.cycle_limit, 
